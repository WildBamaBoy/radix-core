...
public final class DataWatcherEx implements Serializable
{
	...
	public WatchedObjectEx getWatchedObject(int id)
	{
		...
		{
			RadixCore.getLogger().error("Error getting watched object data [" + entityOwner.getClass().getName() + ":" + id + "]");
		}
		...
		try
		{
			for (Object object : watchedObjects.values())
			{
				...
				case String:
					...
					break;
				default:
					RadixCore.getLogger().error("Error writing watched object " + watchableObject.getDataValueId() + " to NBT. Invalid data type.", new Throwable());
					continue;
				}
			}
		}
		...
		try
		{
			for (Object object : watchedObjects.values())
			{
				...
				case String:
					...
					break;
				default:
					RadixCore.getLogger().error("Error reading watched object " + watchableObject.getDataValueId() + " from NBT. Invalid data type.", new Throwable());
					continue;
				}
			}
		}
